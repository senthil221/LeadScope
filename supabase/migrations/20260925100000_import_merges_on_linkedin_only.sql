-- One person, decided by the profile URL and nothing else.
--
-- An import merged a row into an existing candidate on any of four identities:
-- the LinkedIn URL, a Naukri id, an email address, an external id. Three of
-- those are not proof of a person. Recruiting sheets carry a shared team
-- inbox, an assistant's address, an id reused when an account was rebuilt -
-- and on any of them the row's company, designation, CTC and numbers were
-- written straight over somebody else's record, with nothing said about it.
--
-- The profile URL now decides on its own. A row that matches only on an email
-- or a Naukri id is left alone and handed back by name in the import summary,
-- for a person to look at and choose. Nothing is written for it either way.
begin;
