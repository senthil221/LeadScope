-- An import carries both mobile numbers, on the new ten-digit terms.
--
-- Same rule as everywhere else now: no country code, ten digits, and a
-- second number that has to differ from the first. A row whose number cannot
-- be read fails rather than importing with the field quietly dropped,
-- because a candidate nobody can ring is a candidate nobody will work.
begin;
...
begin
 ...
 for r in select * from jsonb_array_elements(p_rows) loop
  ...
  -- Ten digits, and a second number that is not a copy of the first.
  if coalesce(f->>'phone','')<>'' and f->>'phone' !~ '^[0-9]{10}$' then
   n_invalid:=n_invalid+1; continue rows; end if;
  if coalesce(f->>'alternatePhone','')<>'' and (f->>'alternatePhone' !~ '^[0-9]{10}$'
   or f->>'alternatePhone'=f->>'phone') then
   n_invalid:=n_invalid+1; continue rows; end if;
  ...
  if v_candidate is null then
   ...
   insert into public.candidates(full_name,headline,current_company,current_designation,location,
    ...
    values(trim(r->>'name'),coalesce(f->>'headline',''),coalesce(f->>'currentCompany',''),
     ...
     (f->>'totalExperienceYears')::numeric,nullif(f->>'phone',''),
     nullif(f->>'alternatePhone',''),nullif(f->>'email',''),
   ...
   update public.candidates set
    ...
    phone=coalesce(nullif(f->>'phone',''),phone),
    alternate_phone=coalesce(nullif(f->>'alternatePhone',''),alternate_phone),
