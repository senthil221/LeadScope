-- A bulk edit that covers the whole view, not the fifty on screen.
--
-- The cap was fifty because the preview listed every row it would change and
-- fifty is a page. But the edit people actually need is "this whole import
-- went in with the wrong source" - four hundred rows, arrived together, wrong
-- together. Fixing that fifty at a time is eight rounds of preview-and-apply,
-- which is not a smaller version of the task, it is a different and worse one.
--
-- So the selection goes to two thousand and the preview stops listing beyond
-- the first fifty; the counts stay exact, and the whole edit is still one
-- transaction with one snapshot check, so it either all lands or none of it
-- does.
begin;
