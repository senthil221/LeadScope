-- Two numbers for one person, reconciled rather than refused.
--
-- A recruiting sheet lists the same candidate twice more often than not: once
-- with a second number in the alternate column, once with that same number as
-- their primary. Each column was merged against the record on file without
-- looking at the other, so the merged candidate ended up holding one number in
-- both places, the no-duplicate rule fired, and the exception took the rest of
-- the batch with it - reported as a duplicate the recruiter could not find in
-- the file, because it was never in any single row of it.
--
-- The two numbers are now merged together, and the alternate is dropped when
-- it turns out to be the primary again. Same rule within one row.
begin;
