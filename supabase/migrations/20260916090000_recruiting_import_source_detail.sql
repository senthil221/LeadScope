-- Preserve the specific vendor or channel for one import batch. The existing
-- source enum describes the intake method; source_detail gives the recruiter
-- the useful answer, such as "Upwork" or "LinkedIn Recruiter".
begin;

create or replace function private.import_candidates(p_client uuid,p_role uuid,p_rows jsonb,p_source text)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare
 r jsonb; e jsonb; f jsonb; k text; v text; v_source_detail text; ok boolean; n_ident integer;
...
begin
 ...
 for r in select * from jsonb_array_elements(p_rows) loop
  ...
  if r->>'name' is null or length(trim(r->>'name'))=0 or length(r->>'name')>200
    or jsonb_typeof(f)<>'object' or length(v_source_detail)>500 then
  ...
  if v_existed then
   ...
   insert into public.role_candidates(client_id,role_id,candidate_id,source,source_detail)
    values(p_client,p_role,v_candidate,p_source,v_source_detail)
   ...
   if v_role_candidate is not null then
    insert into public.role_candidate_events(client_id,role_candidate_id,kind,to_stage,actor,detail)
     values(p_client,v_role_candidate,'import','all_profiles',auth.uid(),
      jsonb_build_object('source',p_source,'sourceDetail',v_source_detail,'candidateId',v_candidate));
   end if;
  end if;
 end loop;
