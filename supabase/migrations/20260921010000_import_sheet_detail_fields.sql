-- Rows typed or pasted straight into the candidate grid carry the same detail
-- columns the grid shows, so the import has to accept the two fields added for
-- it. Same body as the custom-fields revision, with currentCtc and
-- highestQualification threaded through the insert and the merge.
begin;
